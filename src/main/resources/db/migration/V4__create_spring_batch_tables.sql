-- Spring Batch 6.0.3 메타 테이블 (spring-batch-core 의 schema-mysql.sql 그대로).
-- 탈퇴 계정 하드 삭제 Job(userPurgeJob) 실행 이력을 남긴다 (#76).
-- spring.batch.jdbc.initialize-schema 는 never 로 두고 DDL 은 이 마이그레이션으로만 관리한다.
